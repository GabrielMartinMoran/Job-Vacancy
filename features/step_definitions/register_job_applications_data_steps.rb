When('Applicant {string} applies with expected remuneration {float}') do |_email, expected_remuneration|
  click_link 'Apply'
  fill_in('job_application[expected_remuneration]', with: expected_remuneration)
  click_button('Apply')
end

Then('Email {string} and expected remuneration {float} should be stored for {string} offer') do |_email, expected_remuneration, job_offer_title|
